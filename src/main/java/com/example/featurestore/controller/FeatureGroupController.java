...
public class FeatureGroupController {
    ...
    @PutMapping("/{name}")
    public FeatureGroupResponse update(@PathVariable String name, @Valid @RequestBody UpdateFeatureGroupRequest request) {
        return registryService.update(name, request);
    }

    @GetMapping("/{name}/versions")
    public List<FeatureGroupVersionResponse> versions(@PathVariable String name) {
        return registryService.versions(name);
    }

    @GetMapping("/{name}/versions/{version}")
    public FeatureGroupVersionResponse version(@PathVariable String name, @PathVariable int version) {
        return registryService.version(name, version);
    }
}
